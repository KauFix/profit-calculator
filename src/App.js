...
export default function ProfitCalculator() {
  ...
  const calculateProfit = () => {
    ...
    const emagPerUnit = (selling * emag) / 100;
    const costTotalPerUnit = costPerUnit + emagPerUnit;
    const breakEvenUnits = selling - costTotalPerUnit > 0
      ? Math.ceil((costPerUnit * quantityNum + emagPerUnit * quantityNum) / (selling - costTotalPerUnit))
      : "-";
    ...
    <div className="min-h-screen bg-gray-100 py-8 px-4">
      <div className="max-w-2xl mx-auto bg-white rounded-2xl shadow-xl p-6 space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <InputField label="Preț achiziție China / unitate (RON)" value={pricePerUnit} onChange={setPricePerUnit} icon={<FaTag className="text-blue-500" />} />
          ...
          <InputField label="Transport total din China (RON)" value={transportCost} onChange={setTransportCost} icon={<FaTruck className="text-orange-500" />} />
        ...
        {results && (
          <div className={`mt-4 p-4 rounded-lg text-sm font-medium border ${
            ...
            <p><strong>Punct de echilibru:</strong> {results.breakEvenUnits} unități</p>
          </div>
        )}
